"""
ARCHVIZ WORKFLOW — ITERAZIONE 4
Focus:
- Greener, lush grass ground
- Multi-lobe tree crowns for realistic silhouette
- Garden borders (low stone walls, gravel)
- Better camera angle — more dramatic, lower
- Warm sun color + fill light for shadow recovery
- Higher sample count for final quality
"""
import bpy
import os
import math
...
def make_weathered_concrete(name, tex):
    ...
    el = ramp.color_ramp.elements.new(0.92)
    el.color = (0.80, 0.80, 0.80, 1)
...
# GRASS — much greener, saturated
mat_grass = make_pbr("Grass", tex_grass,
    scale=(8, 8, 8), sat=1.40, val=1.10, contrast=0.12,
    nstr=1.0, bstr=0.08, bdist=0.002, hue=0.48)
...
mat_roof = make_simple("RoofDark", (0.04, 0.04, 0.04, 1), roughness=0.60)
...
# Foliage palette
mat_fd = make_foliage("FolDark", (0.02, 0.08, 0.015, 1))
mat_fm = make_foliage("FolMid", (0.05, 0.14, 0.03, 1))
mat_fl = make_foliage("FolLight", (0.08, 0.20, 0.05, 1))
mat_fh = make_foliage("FolHedge", (0.03, 0.10, 0.02, 1))
...
pav.scale = (4.5, 4.5, 0.025)
bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
pav.data.materials.append(mat_paving)

# Gravel border around paving
for gx, gy, gsx, gsy in [
    (-0.5, -BY/2 - 5.0, 5.0, 0.5),   # Front
    (-3.25, -BY/2 - 2.5, 0.5, 4.5),   # Left
    (2.25, -BY/2 - 2.5, 0.5, 4.5),    # Right
]:
    bpy.ops.mesh.primitive_cube_add(size=1, location=(gx, gy, 0.008))
    g = bpy.context.active_object; g.name = "Gravel"
    g.scale = (gsx, gsy, 0.015)
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
...
w = bpy.context.active_object; w.name = "WallN"; w.scale = (BX, WT, WH)
bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
w.data.materials.append(mat_concrete)

# Left
bpy.ops.mesh.primitive_cube_add(size=1, location=(-BX/2 + WT/2, 0, WH/2 + 0.20))
w = bpy.context.active_object; w.name = "WallW"; w.scale = (WT, BY - 2*WT, WH)
bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
w.data.materials.append(mat_concrete)

# Right — wood
bpy.ops.mesh.primitive_cube_add(size=1, location=(BX/2 - WT/2, 0, WH/2 + 0.20))
w = bpy.context.active_object; w.name = "WallE"; w.scale = (WT, BY - 2*WT, WH)
bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
w.data.materials.append(mat_wood_clad)

# Front left (concrete)
bpy.ops.mesh.primitive_cube_add(size=1, location=(-BX/2 + 1.5, -BY/2 + WT/2, WH/2 + 0.20))
w = bpy.context.active_object; w.name = "WallSL"; w.scale = (3.0, WT, WH)
bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
w.data.materials.append(mat_concrete)

# Front right (wood)
bpy.ops.mesh.primitive_cube_add(size=1, location=(BX/2 - 1.5, -BY/2 + WT/2, WH/2 + 0.20))
w = bpy.context.active_object; w.name = "WallSR"; w.scale = (3.0, WT, WH)
bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
w.data.materials.append(mat_wood_clad)

# Lintel
bpy.ops.mesh.primitive_cube_add(size=1, location=(0, -BY/2 + WT/2, WH - 0.12 + 0.20))
w = bpy.context.active_object; w.name = "Lintel"; w.scale = (6.0, WT, 0.25)
bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
w.data.materials.append(mat_concrete)
...
# Frames
fw = 0.032
for fy, fh, fs in [(0.22, fw, (6.0, 0.05, fw)), (WH - 0.25 + 0.20, fw, (6.0, 0.05, fw))]:
...
sw = bpy.context.active_object; sw.name = "SideWin"; sw.scale = (0.015, 1.5, 1.8)
bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
...
def add_bush(name, pos, scale, mat):
    ...
    nt.noise_scale = 0.20 + random.uniform(-0.08, 0.08)
    nt.noise_depth = 5
...
def add_multi_tree(name, base_pos, height, crown_r, trunk_r, crown_mat):
    """Tree with multi-lobe crown for realistic silhouette."""
    bx, by = base_pos
    # Trunk — slight taper
    bpy.ops.mesh.primitive_cone_add(vertices=10, radius1=trunk_r * 1.3,
                                     radius2=trunk_r * 0.6, depth=height,
                                     location=(bx, by, height/2))
    t = bpy.context.active_object; t.name = f"T_{name}"
    t.data.materials.append(mat_bark)

    # Crown = 5-7 overlapping spheres at various positions
    ...
    for li in range(n_lobes):
        ...
        bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=2, radius=lr,
                                               location=(lx, ly, lz))
...
for hi in range(9):
    hx = -BX/2 + 0.5 + hi * 1.5
    add_bush(f"H{hi}", (hx, BY/2 + 1.8, 0.55), 0.80, mat_fh)
...
# Fill light (subtle, from camera side, to lift shadows)
bpy.ops.object.light_add(type='SUN', location=(0, 0, 10))
fill = bpy.context.active_object; fill.name = "Fill"
fill.data.energy = 0.6
fill.data.angle = math.radians(15)
fill.rotation_euler = (math.radians(60), 0, math.radians(30))
fill.data.color = (0.85, 0.90, 1.0)  # Slightly cool
...
out_path = os.path.join(OUT_DIR, "iterazione_4.png")
scene.render.filepath = out_path

print("\n=== ARCHVIZ ITER 4 — Multi-lobe trees, green garden, golden hour ===")
